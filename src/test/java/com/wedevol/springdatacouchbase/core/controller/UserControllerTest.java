...
public class UserControllerTest {
  ...
  public void getNonExistingUser() throws Exception {
    mockMvc.perform(get("/users/100")).andExpect(status().isNotFound()).andExpect(content().contentType(CONTENT_TYPE));
  }
  ...
  public void findAll() throws Exception {
    mockMvc.perform(get("/users/find/all")).andExpect(status().isOk()).andExpect(content().contentType(CONTENT_TYPE))
        .andExpect(jsonPath("$", Matchers.hasSize(3)));
  }

  @Test
  public void countAll() throws Exception {
    mockMvc.perform(get("/users/count/all")).andExpect(status().isOk()).andExpect(content().contentType(CONTENT_TYPE))
